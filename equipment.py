...
class Equipment(object):
    def __init__(self):
        ...
        self.locked = True
        self.slot = [Slot(), Slot(), Slot(), Slot()]
        
    def unlock(self):
        """unlocks the equipment"""
        self.locked = False

    #returns 0 normally. returns -1 if item is completely unlocked. not used.
    def unlock_slot(self):
        """unlock the next slot"""
        for i in range(0, 4):
            if self.slot[i].unlocked == True:
                self.slot[i].locked = False
                return 0
