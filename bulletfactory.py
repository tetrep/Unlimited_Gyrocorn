...
class BulletFactory():
    def __init__(self):
        ...
        self.attack_sound = pygame.mixer.Sound("Music/turretattack.ogg")
    ...
    def createBullet(self, game, type, area_of_effect, damage, range, target, starting_x = 0, starting_y = 0):
        ...
        # Basic Bullet
        if type == 0:
            return Bullet(game, self.imgBasicBullet, self.attack_sound, 0 + area_of_effect, 2 + damage, 2 + range, 1, target, starting_x, starting_y)
        # Sniper Bullet
        elif type == 1:
            return Bullet(game, self.imgSniperBullet, self.attack_sound, 0 + area_of_effect, 30 + damage, 2 + range, 1, target, starting_x, starting_y)
        # Piercing Bullet
        elif type == 2:
            return PiercingBullet(game, self.imgPiercingBullet, self.attack_sound, 0 + area_of_effect, 10 + damage, 2 + range, 1, target, starting_x, starting_y)
        # Mortar Bullet
        elif type == 3:
            return MortarBullet(game, self.imgMortarBullet, self.attack_sound, 20 + area_of_effect, 8 + damage, 2 + range, 1, target, starting_x, starting_y)
        # Paralyze Bullet
        elif type == 4:
            return ParalyzeBullet(game, self.imgParalyzeBullet, self.attack_sound, 0 + area_of_effect, 0 + damage, 2 + range, 1, target, starting_x, starting_y)
        # Fire Bullet
        elif type == 5:
            return FireBullet(game, self.imgFireBullet, self.attack_sound, 0 + area_of_effect, 10 + damage, 2 + range, 2, target, starting_x, starting_y)
        # Ice Bullet
        elif type == 6:
            return IceBullet(game, self.imgIceBullet, self.attack_sound, 0 + area_of_effect, 10 + damage, 2 + range, 3, target, starting_x, starting_y)
        # Lightning Bullet
        elif type == 7:
            return LightningBullet(game, self.imgLightningBullet, self.attack_sound, 0 + area_of_effect, 10 + damage, 2 + range, 4, target, starting_x, starting_y)