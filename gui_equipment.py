...
class GUI_Equipment(object):
    ...
    def generate_GUI_objects(self):
        ...
        #stat totals
        x = g.screenSize[0] - 512 - 24
        y = 0
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "STATS" ] ]
        self.objects.append( temp )
        #HP
        y += 24
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "HP: ", self.g.players[self.PID].hp[1] ] ]
        self.objects.append( temp )
        #regen/sec
        y += 24
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "  + ", self.g.players[self.PID].regen, " HP/s" ] ]
        self.objects.append( temp )
        #defense %
        y += 24
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "  -", (1.0 - (100.0 / float(self.g.players[self.PID].defense) ) ) * 100, "% Damage Taken" ] ]
        self.objects.append( temp )
        #absorbtion
        y += 24
        absorb = self.g.players[self.PID].absorbtion / 2.0
        absorb += (self.g.players[self.PID].absorbtion / 2.0) / (100.0 / float(self.g.players[self.PID].defense) )
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "  -", absorb, "  Damage Taken" ] ]
        self.objects.append( temp )
        #damage (includes IAS, crit)
        y += 48
        attack = self.g.players[self.PID].attack
        attack *= (self.g.players[self.PID].crit / 100.0 + 1)
        attack *= (self.g.players[self.PID].attackSpeedMultiplier)
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "Attack: ", attack ] ]
        self.objects.append( temp )
        #attack speed
        y += 24
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "  ", self.g.players[self.PID].attackSpeedMultiplier, "x Attack Speed" ] ]
        self.objects.append( temp )
        #crit
        y += 24
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "  ", self.g.players[self.PID].crit, "% Critical Hit Chance" ] ]
        self.objects.append( temp )
        #lifesteal
        y += 24
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "  ", self.g.players[self.PID].lifeLeech, " % Damage Stolen as Life" ] ]
        self.objects.append( temp )
        #move speed
        y += 24
        temp = GUIObject(x, y, 256, 24, self.null_action, [])
        temp.text = [ [ "Move Speed: ", self.g.players[self.PID].moveSpeedMultiplier, "x" ] ]
        self.objects.append( temp )
    ...
    def slot_action(self, arg):
        ...
        if g.players[arg[0]].equipment[arg[1]].locked == True:
            ...
            if g.players[arg[0]].equipment[arg[1]].slot[arg[2]].locked == True and g.players[arg[0]].exp > 100:
                ...
                cost = g.players[arg[0]].equipment[0].slot[0].get_price( code, tier + 1 )
                ...
                if g.players[arg[0]].equipment[arg[1]].slot[arg[2]].modCode == 0:
                    ...
                    #update player stats
                    self.g.players[self.PID].update_stats()
                    #update text
                    del self.objects[:]
                    self.generate_GUI_objects()
